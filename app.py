...
@app.route("/roadmap/print")
@login_required
def print_roadmap():
    data = session.get("last_result")
    user_input = session.get("last_input")
    if not data:
        flash("No recent prediction to print. Please generate a result first.", "error")
        return redirect(url_for("dashboard"))
    return render_template("print_roadmap.html", result=data, user_row=user_input)
